/**
 * @file run-migration-0023.ts
 * @description Applies migration 0023 (expenses table) to the live database
 * using a raw pg client.
 *
 * Usage:
 *   npx tsx script/run-migration-0023.ts
 *
 * All DDL statements use IF NOT EXISTS so re-running is safe.
 */
...
const MIGRATION_FILE = resolve(__dirname, "../migrations/0023_expenses_table.sql");
const LOG_PREFIX = "[Migration 0023]";
...
async function run(): Promise<void> {
  ...
  try {
    ...
    console.log(`${LOG_PREFIX} ✅ Migration 0023 applied successfully.`);
  } catch (err: unknown) {
